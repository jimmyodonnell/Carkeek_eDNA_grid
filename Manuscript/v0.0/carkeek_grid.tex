...
For an immobile organism (e.g. fungal hyphae) % effectively 
in a static environmental medium (e.g. soil), %n effectively
the relationship between DNA concentration and distance appears straightforward (CITE ???).
But the relationship may be complicated by a dynamic environment: %One might expect the relationship to break down in 
...
because the robustness of eDNA surveys for detecting and quantifying organisms depends on the distance that DNA travels from a source. %in aquatic environments 
