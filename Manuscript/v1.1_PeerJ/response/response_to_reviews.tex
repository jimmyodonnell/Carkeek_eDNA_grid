...
\textbf{Line numbers refer to the track changes version of the manuscript}.
...
\begin{response}
  Thank you for the positive feedback.
	...
	Like you, we were curious about the effect of the data subset you mention, and conducted that analysis using the life history data. Unfortunately, we found no meaningful patterns, and have made an attempt to clarify this in the text (L356).
	\\
\end{response}
...
\begin{response}
  Thanks for catching these errors. We made these changes (L99, L100).\\
\end{response}
...
\begin{response}
  Excellent suggestion; we made this change (L98).\\
\end{response}

L93: ``which provide''
\begin{response}
  We made this change (L102).\\
\end{response}
...
\begin{response}
  We did not mean to imply that the microscopic plankton is composed solely of diatoms and larve, we made a  statement and qualified it with two examples to give meaning and context: `a diverse assemblage of microscopic plankton including diatoms and larvae'.
	...
	We added copepods to this list, though we recognize that it is necessarily incomplete (L104).\\
\end{response}
...
\begin{response}
  Indeed, our wording did not clearly convey our meaning. Our intent was to indicate the novelty of the experimental design which was intended to capture relatively fine-scale spatial distribution of eDNA in marine waters. We have made the suggested change (L110).\\
\end{response}
...
\begin{response}
  We intended `genomic eDNA' to mean the full DNA extracted from samples, but your suggestion is a good one and we have implemented it (L182).\\
\end{response}

L171 to 172. Use ``95 ºC'' instead of ``95C''.
\begin{response}
  We made this change (L154, L159, L162, L184, L185).\\
\end{response}

L195. ``(machine NS500615, run 115, flowcell H3LFLAFXX)''. I don't think we need to know these details!
\begin{response}
  While this does seem like overkill, it is not unheard of for machines or sequencing facilities to develop problems that can bias data, and we included this with the intent of being as transparent as possible. Nevertheless, we removed this information (L209).\\
\end{response}
...
\begin{response}
  We are grateful that you have read the manuscript with such a careful eye!
	...
	Similar to your statement, the \textbf{less} PhiX is spiked-in, the fewer clusters will be correctly differentiated from one another, and the fewer reads passing filter will be obtained, thus also decreasing sequencing depth (L210).\\
\end{response}

L223. Delete ``a'' in ``This allows for a samples''.
\begin{response}
  We made this change (L237).\\
\end{response}
...
\begin{response}
  You make an excellent point that we were not clear in our wording.
	...
	We have attempted to clarify this in the text (L284).
	...
	We added a concluding sentence to the paragraph to explain the reason for these data (L292).
...
\begin{response}
  Our intent was precisely to convey the novelty of the sampling design; we failed to do that clearly.
	We removed `For the first time in a marine environment' (L264), and we have clarified the novelty of our study in the context of recently published work in the introduction (L76).\\
\end{response}
...
\begin{response}
  It is unclear what the specific error is, but we acknowledge this sentence is poorly worded.
	We changed this to read `We demonstrate that distant locations have less-similar eDNA communities than proximate locations in Puget Sound, a dynamic marine environment' (L371).\\
\end{response}
...
\begin{response}
  This is a good suggestion; we have modified the abstract to clarify this as early as possible (L9).\\
\end{response}
...
\begin{response}
  We added a brief explanation of our marker choice to the section Methods:PCR Amplification (L166), and a more general discussion of marker choice to the discussion (L452).\\
\end{response}
...
\begin{response}
  Thank you for noticing this; we have corrected the legend to be meaningful as a standalone figure (Figure 5 legend).\\
\end{response}
...
\begin{response}
  Yes; these links were a placeholder until the manuscript is accepted and have now been updated (L514).\\
\end{response}
...
\begin{response}
  Perhaps we misunderstand your comment, but in the relevant part of the methods section, we state: `Each environmental sample was collected in a clean 1 liter high-density polyethylene bottle, the opening of which was covered with 500 micrometer nylon mesh to prevent entry of larger particles' (L140).\\
\end{response}
...
\begin{response}
  In our initial draft, we provided an outline of the bioinformatics procedures in the section Methods:Sequence Data Processing (Bioinformatics) (L213), and document the procedures in painstaking detail in the Supplemental Material section Methods:Bioinformatics (L725).
	However, we overlooked the inclusion of a direct link to the scripts used to process the raw sequence data, and have now added those (L515).
...
\begin{response}
  This is an excellent point. We have modified the relevant section to incorporate this potential mechanism (L411).
	\\
\end{response}
...
\begin{response}
  We believe we adequately addressed alpha diversity by reporting richness and evenness both in a 2-dimensional and linear spatial context, with accompanying figures.
	However, we have now also added an OTU accumulation figure to the supplement (Figure 10) and commented on it in the results (L310).
