...
%DIF DEL ../v1.0_PeerJ/carkeek_grid.tex   Mon Nov 28 11:21:51 2016
%DIF ADD carkeek_grid.tex                 Fri Jan 13 13:28:48 2017
%DIF 2-3d2
%DIF < % (TODO SUBMIT DATA AND METADATA; ADD URL)
%DIF < % (TODO INSERT DOI FROM FINAL COMMIT BEFORE PUBLICATION)
%DIF -------
...
%DIF 40c38-40
%DIF < \usepackage{nameref,hyperref} % named references (e.g. "Supplemental Material")
%DIF -------
\usepackage{nameref} % named references (e.g. "Supplemental Material") %DIF >
\usepackage{breakcites} % add line breaks in citations where necessary %DIF >
\usepackage[breaklinks=true]{hyperref} %DIF >
%DIF -------
...
%DIF 59-61c59-61
%DIF < \author[2]{Jameal F. Samhouri} % jameal.samhouri@noaa.gov
%DIF < \author[1,3]{Natalie C. Lowell} % nclowell@uw.edu
%DIF < \author[4]{Gregory D. Williams} % greg.williams@noaa.gov
%DIF -------
\author[3]{Jameal F. Samhouri} % jameal.samhouri@noaa.gov %DIF >
\author[1,4]{Natalie C. Lowell} % nclowell@uw.edu %DIF >
\author[5]{Gregory D. Williams} % greg.williams@noaa.gov %DIF >
%DIF -------
...
%DIF 65-67c65-68
%DIF < \affil[2]{Northwest Fisheries Science Center, NOAA Fisheries, 2725 Montlake Blvd E, Seattle, Washington 98112, USA}
%DIF < \affil[3]{School of Aquatic and Fishery Sciences, University of Washington, 1122 NE Boat St, Seattle, Washington 98105, USA}
%DIF < \affil[4]{Pacific States Marine Fisheries Commission, Under contract to the Northwest Fisheries Science Center, NOAA Fisheries, 2725 Montlake Blvd E, Seattle, WA 98112}
%DIF -------
\affil[2]{Earth Resource Technology, Inc., Under contract to the Northwest Fisheries Science Center, National Marine Fisheries Service, National Oceanic and Atmospheric Administration, 2725 Montlake Blvd E, Seattle, WA 98112, USA} %DIF >
\affil[3]{Conservation Biology Division, Northwest Fisheries Science Center, National Marine Fisheries Service, National Oceanic and Atmospheric Administration, 2725 Montlake Blvd E, Seattle, Washington 98112, USA} %DIF >
\affil[4]{School of Aquatic and Fishery Sciences, University of Washington, 1122 NE Boat St, Seattle, Washington 98105, USA} %DIF >
\affil[5]{Pacific States Marine Fisheries Commission, Under contract to the Northwest Fisheries Science Center, National Marine Fisheries Service, National Oceanic and Atmospheric Administration, 2725 Montlake Blvd E, Seattle, WA 98112, USA} %DIF >
%DIF -------
...
Using \DIFdelbegin \DIFdel{massively parallel sequencing of }\DIFdelend \DIFaddbegin \DIFadd{PCR primers that target a diverse assemblage of metazoans, we amplified a region of mitochondrial }\DIFaddend 16S \DIFdelbegin \DIFdel{amplicons, we identified a diverse community of metazoans and quantified }\DIFdelend \DIFaddbegin \DIFadd{rDNA from the samples and sequenced the products on an Illumina platform in order to detect communities and quantify }\DIFaddend their spatial patterns using a variety of statistical tools.
...
\DIFaddbegin \DIFadd{Research to date has documented the non-random spatial distribution of %DIF >  spatial variation of
meiofaunal \mbox{%DIFAUXCMD
\citep{Fonseca2014,Guardiola2016}}%DIFAUXCMD
, microbial \mbox{%DIFAUXCMD
\citep{Lallias2015}}%DIFAUXCMD
,
and extracellular \\
\mbox{%DIFAUXCMD
\citep{Guardiola2015} }%DIFAUXCMD
eDNA of marine and estuarine sediments, and of microscopic plankton in open ocean waters \mbox{%DIFAUXCMD
\citep{DeVargas2015}}%DIFAUXCMD
.
...
After assigning taxon names to the abundance data, we plotted the distribution in space of a selection of taxa to compare with our expectations on the basis of adult distributions (objective 2). Our aim was to understand where each taxon occurred in the greatest proportional abundance, and its distribution in space relative to that maximum. Thus, we rescaled each sample to proportional abundance, extracted the data from a single taxon, and scaled those values between 0 and 1. We collated life history characteristics for each of the major taxonomic groups recovered, including dispersal range of the gametes, larvae, and adults, adult habitat type and selectivity, and adult body size. \DIFdelbegin \DIFdel{Dispersal range was given as }\DIFdelend \DIFaddbegin \DIFadd{For each life history stage of each taxon group, we made }\DIFaddend an order-of-magnitude approximation of the scale of dispersal\DIFdelbegin \DIFdel{: for }\DIFdelend \DIFaddbegin \DIFadd{. For }\DIFaddend example, internally fertilized species were assigned a gamete range of 0 km, while broadcast spawners were assigned a gamete range of 10 km. Similarly, adult range size was approximated as 0 km (sessile), 1 km (motile but not pelagic), or 10 km (highly mobile, pelagic). Variables were specified as 'multiple' for \DIFdelbegin \DIFdel{groups }\DIFdelend \DIFaddbegin \DIFadd{life history stages }\DIFaddend known to span more than 1 magnitude of range size. For groups to which sequences were annotated with high confidence, but for which life history strategy is diverse or poorly known (e.g. families in the phylum Nemertea), we used conservative, coarse approximations at a higher taxonomic rank (see \nameref{life_history_data}). \DIFaddbegin \DIFadd{These data were used to contextualize group-specific spatial distributions and inform expectations based on known adult distributions.
}\DIFaddend
...
\DIFaddbegin \DIFadd{Finally, the uncertainty in identification of the number of distinct clusters to best characterize the community underlines the difficulty of identifying community patterns with the number of taxonomic groups considered here. We suspect that the signature of eDNA from microscopic life-stages may explain our inability to easily detect spatial community level patterns that align with our initial expectations.
}\DIFaddend

 \subsubsection*{(3) Richness declines and evenness increases with distance from shore}
We \DIFdelbegin \DIFdel{detected a general pattern of declining richness and increasing evenness with increasing distance offshore}\DIFdelend \DIFaddbegin \DIFadd{found that richness declined while evenness increased with distance from shore}\DIFaddend .
...
\DIFdelbegin \DIFdel{The coastal ocean is a highly productive and diverse ecosystem \mbox{%DIFAUXCMD
\citep{Ray1988}}%DIFAUXCMD
. }\DIFdelend However, our study is novel in that it corroborates a cline well-known on macroscales for macrobiota on a much smaller spatial scale for microscopic animals, suggesting that there may be a self-similarity across scales in diversity patterning \citep{Levin1992}.
\DIFaddbegin \DIFadd{The coastal ocean is a highly productive and diverse ecosystem, where biomass is concentrated most heavily along the bottom and shoreline \mbox{%DIFAUXCMD
\citep{Ray1988}}%DIFAUXCMD
.
...
}\DIFaddend Intriguingly, the cline in diversity from inshore to offshore was not determined by shared changes in communities as one moved offshore; the classification analysis suggested a fair amount of differences among communities at a given offshore distance (Figure \ref{pam_in_space}).
\DIFdelbegin \DIFdel{Furthermore, the uncertainty in identification of the number of distinct clusters to best characterize the community underlines the difficulty of identifying community patterns with the number of taxonomic groups considered here. We suspect that the signature of eDNA from microscopic life-stages may explain our inability to easily detect spatial community level patterns that align with our initial expectations.
}\DIFdelend


\subsubsection*{(4) Spatial patterning of eDNA is associated with taxon-specific life history\DIFdelbegin \DIFdel{characteristics.}\DIFdelend }
...
\DIFaddbegin \DIFadd{The marker we chose for this study detects a wide variety of metazoans while excluding other more common taxa; however, it does not effectively discriminate among species within a higher group in all cases.
Other markers, such as mitochondrial cytochrome c oxidase subunit 1 (COX1, CO1, or COI) may provide adequate species-level resolution in some metazoan groups, but have other shortcomings including taxon dropout \mbox{%DIFAUXCMD
\citep{Deagle2014} }%DIFAUXCMD
and amplification of more abundant non-metazoans, as we discovered in an accompanying study \mbox{%DIFAUXCMD
\citep{Kelly2016FMS}}%DIFAUXCMD
.
...
In the case of COX1, well-documented primer biases cause failure to amplify some taxa, particularly in mixed samples, with the same result \mbox{%DIFAUXCMD
\citep{Deagle2014}}%DIFAUXCMD
.
...
We wish to thank \DIFaddbegin \DIFadd{Linda Park, }\DIFaddend Robert Morris, E. Virginia Armbrust, and James Kralj.
\DIFaddbegin \DIFadd{The manuscript was improved by suggestions from editor Magnus Johnson, and reviewers Owen Wangensteen and Stephen Moss.
}\DIFaddend
...
\DIFdelbegin \subsection{\DIFdel{Sequence Data}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection*{\DIFadd{Sequence Data}}
\DIFaddend \label{sequence_data}
All sequence \DIFdelbegin \DIFdel{files and metadata }\DIFdelend \DIFaddbegin \DIFadd{data }\DIFaddend are available from \DIFdelbegin \DIFdel{EMBL}\DIFdelend \DIFaddbegin \DIFadd{NCBI under BioProject PRJNA338801.}\\
\DIFadd{Scripts to process raw sequence data into the contingency tables used for ecological analyses can be found at}\DIFaddend : \\
\DIFdelbegin %DIFDELCMD < \verb!http://www.ebi.ac.uk/ena/data/view/FIXME!
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \verb!https://github.com/jimmyodonnell/banzai!\\
\DIFaddend

\DIFdelbegin \subsection{\DIFdel{Project Repository}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection*{\DIFadd{Project Repository}}
\DIFaddend The following components are available from the project repository on GitHub: \\
\verb!https://github.com/jimmyodonnell/Carkeek_eDNA_grid!\\
\DIFdelbegin %DIFDELCMD < \verb!http://dx.doi.org/FIXME!
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \verb!https://doi.org/10.5281/zenodo.242976!
\DIFaddend

\DIFdelbegin \subsubsection{\DIFdel{Sequencing Metadata}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsubsection*{\DIFadd{Sequencing Metadata}}
\DIFaddend \label{sequencing_metadata}
Sequencing metadata is available in: \verb!Data/metadata_spatial.csv!

\DIFdelbegin \subsubsection{\DIFdel{Life History Data}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsubsection*{\DIFadd{Life History Data}}
\DIFaddend \label{life_history_data}
Life history data is available in: \verb!Data/life_history.csv!

\DIFdelbegin \subsubsection{\DIFdel{Analysis Scripts}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsubsection*{\DIFadd{Analysis Scripts}}
\DIFaddend \label{analysis_scripts}
All analyses were performed using scripts available in the Analysis subdirectory \DIFaddbegin \DIFadd{of the project's repository on GitHub}\DIFaddend .
...
\begin{figure}[H] %DIF >  [h!] forces the figure to be placed roughly here
  \centering
